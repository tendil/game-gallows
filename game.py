...
class Game:
	...
	def __init__(self, allowed_misses: int = 6):
		if allowed_misses < 5 or allowed_misses > 8:
			raise ValueError("Number of  allowed misses should be between 5 and b.")
	...
	def generate_word(self) -> str:
		...
		with open('wordsRand.txt', encoding='utf8') as file:
			for line in file:
	...
	def guess_letter(self, letter: str) -> Iterable[str]:
		if self.tries_counter == self.allowed_misess:
			raise InvalidOperationError(f'Exceeded the max misses number. Allowed {self.allowed_misess}')
		...
		elif self.tries_counter == self.allowed_misess:
	...
	def remaining_tries(self) -> int:
		return self.allowed_misess - self.tries_counter
